...
begin
  ...
  foreach destination in array p_destination_ids loop
    ...
    insert into public.publishing_jobs(organisation_id,post_destination_id,status,scheduled_for,next_attempt_at,idempotency_key)
      values(p_organisation_id,new_pd,case when p_scheduled_for<=now() then 'ready'::public.job_status else 'scheduled'::public.job_status end,p_scheduled_for,p_scheduled_for,encode(digest(new_post::text||':'||destination::text,'sha256'),'hex'));
  end loop;
...
begin
  ...
  values(p_organisation_id,trim(p_name),p_type,p_os,case when p_type='chrome_extension' then p_version end,case when p_type='desktop_agent' then p_version end,encode(digest(raw_token,'sha256'),'hex'),(select auth.uid())) returning id into device_id;
...
create or replace function public.job_action(p_job_id uuid,p_action text,p_value text default null) returns public.job_status
language plpgsql security invoker set search_path='' as $$
declare current_status public.job_status; org uuid; next_status public.job_status;
begin
  select status,organisation_id into current_status,org from public.publishing_jobs where id=p_job_id for update;
  if not public.has_org_role(org,array['owner','admin','editor']::public.member_role[]) then raise exception 'not authorised'; end if;
  next_status=case p_action when 'retry' then 'ready' when 'skip' then 'skipped' when 'cancel' then 'cancelled' when 'review' then 'requires_attention' else null end;
  if next_status is null then raise exception 'invalid action'; end if;
  if current_status='published' then raise exception 'published jobs are immutable'; end if;
  if p_action='retry' and current_status not in ('failed','requires_attention') then raise exception 'job is not retryable'; end if;
  update public.publishing_jobs set status=next_status,next_attempt_at=case when p_action='retry' then now() else next_attempt_at end,
    claimed_by=null,claimed_at=null,lease_expires_at=null,last_error_message=case when p_action='review' then coalesce(p_value,last_error_message) else last_error_message end where id=p_job_id;
  insert into public.activity_logs(organisation_id,actor_id,action,entity_type,entity_id,summary) values(org,(select auth.uid()),p_action,'publishing_job',p_job_id::text,'Job action: '||p_action);
  return next_status;
end $$;
grant execute on function public.job_action(uuid,text,text) to authenticated;
